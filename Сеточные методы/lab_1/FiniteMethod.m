function y = FiniteMethod(n, a, b, dim)
  h = (b - a) / (n - 1);
  x = a : h : b;
  n = length(x);
  r = zeros(1, n);
  ...
  if dim == 1
    r(1) = (e ^ 0.5 * cos(e ^ 0.5) + sin(e ^ 0.5)) * h;
    r(n) = (e ^ 2 * cos(e ^ 2) + sin(e ^ 2)) * h;
    matrix(1, 1) = -alpha(1);
    matrix(2, 1) = alpha(1) + alpha(2) * h;
    matrix(3, n) = -beta(1);
    matrix(2, n) = beta(1) + beta(2) * h;
  else
    r(1) = 2 * (e ^ 0.5 * cos(e ^ 0.5) + sin(e ^ 0.5)) * h + ...
    + alpha(1)*(h^2*f(x(2)))/(1 - h*p(x(2))/2);
    r(n) = 2 * ((e ^ 2) * cos(e ^ 2) + sin(e ^ 2)) * h + ...
    + beta(1)*h^2*f(x(n-1))/(1 + h*p(x(n-1))/2);
